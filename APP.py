...
class UltraSQLAgent:
    ...
    def process_query(self, question: str, examples: str = ""):
        ...
        {{"intent": "GENERAL_CHAT", "language": "ar", "sql_query": null, "relevant_tables": null, "chat_response": "Your reply here should be in the same language as the user."}}
        ...
        6. For ROUND() with decimal places, always cast to NUMERIC first:
           ROUND(value::NUMERIC, 2) not ROUND(value, 2)
    ...
    with st.spinner("Thinking..."):
        ...
        if result.intent == "SECURITY_VIOLATION":
            ...
            try:
                ...
                Total rows returned: {total_rows}. Displayed: {min(DISPLAY_LIMIT, total_rows)}.
                in {result.language}.
                Be concise - write 2-3 sentences maximum summarizing the key insights only.
                If total rows > {DISPLAY_LIMIT}, mention that only the first {DISPLAY_LIMIT} rows are shown.
                ...
                with st.chat_message("assistant"):
                    st.markdown(response)
                    st.caption(f"Showing {min(DISPLAY_LIMIT, total_rows)} of {total_rows} rows")
                    st.dataframe(df_display, use_container_width=True)

                    if total_rows > DISPLAY_LIMIT:
                        st.info(
                            f"⚠️ Query returned {total_rows} rows. "
                            f"Showing first {DISPLAY_LIMIT} only. "
                            f"Ask for the next batch if needed."
                        )
